...
# ///
# this_file: adamdubpy.py

"""
adamdubpy - Voice dubbing tool using ElevenLabs speech-to-speech API.
...
# Import our modules
from audio import AudioExtractor, SilenceAnalyzer, AudioSegmenter, AudioProcessor
...
# Constants
DEFAULT_VOICE_ID = "iBR3vm0M6ImfaxXsPgxi"
...
class AdamDubPy:
    ...
    def process(self, input: str, voice: str = DEFAULT_VOICE_ID, output: Optional[str] = None):
        ...
        if not self.temp_manager.check_disk_space("/tmp", estimated_space):
            console.print(f"[red]Error: Insufficient disk space. Need ~{estimated_space:.1f}GB[/red]")
            sys.exit(1)
        ...
        with self.temp_manager.temp_directory() as temp_dir:
            try:
                ...
                converted_segments = []
                with self.progress_tracker.track_segments(len(segment_paths), "Converting segments") as update:
                    for i, (segment_path, (start, end)) in enumerate(zip(segment_paths, segments)):
                        ...
                        self.elevenlabs_client.speech_to_speech(
                            segment_path,
                            voice,
                            converted_path
                        )
                ...
                with self.progress_tracker.track_file_operation("Reassembling audio"):
                    final_audio = self.audio_segmenter.concatenate(
                        converted_segments,
                        os.path.join(temp_dir, "final_audio.wav")
                    )
...
def main(input: str, voice: str = DEFAULT_VOICE_ID, output: Optional[str] = None, verbose: bool = False):
    """adamdubpy - Voice dubbing tool using ElevenLabs speech-to-speech API.
    ...
    Args:
        ...
        verbose: Enable verbose logging output
    """
    dubber = AdamDubPy(verbose=verbose)
