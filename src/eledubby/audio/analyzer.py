...
class SilenceAnalyzer:
    ...
    def analyze(self, audio_path: str, min_duration: float = 10.0, 
    ...
    def _calculate_silence_scores(self, audio_data: np.ndarray, 
        ...
        for i in range(0, len(audio_data) - window_size, hop_size):
            ...
            if rms < threshold_linear:
                silence_score = 1.0 - (rms / threshold_linear)
            else:
                silence_score = 0.0
    ...
    def _find_optimal_segments(self, silence_scores: List[Tuple[float, float]],
                               sample_rate: int, min_duration: float,
                               max_duration: float, total_duration: float) -> List[Tuple[float, float]]:
        ...
        while current_start < total_duration:
            ...
            for time, score in silence_scores:
                if window_start <= time <= window_end:
                    ...
                    position_weight = 1.0 - abs(time - (window_start + window_end) / 2) / (max_duration / 2)
